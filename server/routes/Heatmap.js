...
var hm=require('../models/Heatmap');
  ...
  hm.choosen1(req.params.camera_id,req.params.st,req.params.end,function(err,rows){
    if(err)
    {
    res.json(err);
    }
  ...
  router.get('/first/:st/:end/:dwell/:camera_id',function(req,res,next){
    hm.dwell1(req.params.camera_id,req.params.st,req.params.end,req.params.dwell,function(err,rows){
      if(err)
      {
      res.json(err);
      }
