...
var Relations = {
  getRels: function (uuid, callback) {
    return db.query(
      'SELECT * FROM relations WHERE camera_id = ? and snippet_id is null',
      [uuid],
      callback,
    )
  },
  ...
  getRelsFromSnippetId: function (uuid, callback) {
    return db.query(
      'SELECT * FROM relations WHERE snippet_id = ?',
      [uuid],
      callback,
    )
  },
  ...
  delete: function (id, callback) {
    return db.query(
      'DELETE FROM relations WHERE snippet_id = ?',
      [id],
      callback,
    )
  },
  ...
  deleteByCameraId: function (cameraId, callback) {
    return db.query(
      'DELETE FROM relations WHERE camera_id = ?',
      [cameraId],
      callback,
    )
  },
